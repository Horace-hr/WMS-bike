...
<script>
			...
			function(request, response){
				...
				$.ajax({
					...
					success : function(data){
						...
						stockin_goods = data.rows[0].id; //设定全局变量stockin_goods的编号
					},
	...
	$('#supplier_input').blur(
			function(request, response){
				var keyWord = $('#supplier_input').val();
				if('' == keyWord)
					return;
				$.ajax({
					type : 'GET',
					...
					success : function(data){
						...
						supplierInfoSet(supplierCache);
						stockin_supplier = data.rows[0].id;
					},
					error : function(XMLHttpRequest, textStatus, errorThrown) { 
					} 
	...
	// 填充供应商详细信息
	function supplierInfoSet(supplierCache){
		var detailInfo;
		$.each(supplierCache,function(index,elem){
			console.log('打印供应商详细信息'+JSON.stringify(elem));
				detailInfo = elem;
	...
	function goodsInfoSet(goodsCache){
		...
		$.each(goodsCache,function(index,elem){
			console.log('打印货物详细信息'+JSON.stringify(elem));
				detailInfo = elem;
				if(detailInfo.id==null)
					$('#info_goods_ID').text('-');
				else
